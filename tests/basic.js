(function($) {
    ...
    test('Test Default Options', function(assert) {
        ...
        assert.equal($.fn.formset.defaults.formsetClone, null, 'formsetClone: null');
        assert.equal($.fn.formset.defaults.addText, '<i class="icon-plus-sign icon-large"></i>', 'addText: "add another"');
        assert.equal($.fn.formset.defaults.deleteText, '<i class="icon-remove-sign icon-large"></i>', 'deleteText: "remove"');
    ...
    test('Test Formset Creation', function(assert) {
        $('#stacked-form div').each((i, el) => {
            console.log(i, el)
        })
        assert.equal($('#stacked-form .dynamic-form').length, 1, 'Default form is present.');
